...
MIN_HISTORY_ITEMS = 3
INGREDIENT_EMBEDDING_COLUMNS = [
    f"ingredient_emb_{idx}" for idx in range(1, EMBEDDING_COMPONENTS + 1)
]
TEXT_EMBEDDING_COLUMNS = [
    f"text_emb_{idx}" for idx in range(1, EMBEDDING_COMPONENTS + 1)
]
HISTORY_INGREDIENT_EMBEDDING_COLUMNS = [
    f"history_ingredient_emb_{idx}" for idx in range(1, EMBEDDING_COMPONENTS + 1)
]
HISTORY_TEXT_EMBEDDING_COLUMNS = [
    f"history_text_emb_{idx}" for idx in range(1, EMBEDDING_COMPONENTS + 1)
]
...
def ensure_feature_columns(instance: dict[str, Any]) -> dict[str, Any]:
    ...
    for col in FEATURE_COLUMNS:
        enriched.setdefault(col, _default_feature_value(col))
    return enriched
...
class EmbeddingPreprocessor:
    def __init__(self, ingredient_pipeline: dict[str, Any] | None, text_pipeline: dict[str, Any] | None):
    ...
    def from_artifact_dir(cls, artifact_dir: str | Path | None) -> "EmbeddingPreprocessor | None":
        ...
        if not ingredient_path.exists() or not text_path.exists():
            return None
        try:
            with open(ingredient_path, "rb") as handle:
                ingredient_pipeline = pickle.load(handle)
    ...
    def _pipeline_transform(self, documents: list[str], pipeline: dict[str, Any] | None) -> np.ndarray:
        if not pipeline or not pipeline.get("vectorizer") or not pipeline.get("svd"):
        ...
        tfidf = pipeline["vectorizer"].transform(normalized)
        matrix = pipeline["svd"].transform(tfidf)
        if matrix.shape[1] < EMBEDDING_COMPONENTS:
            matrix = np.pad(
                matrix,
                ((0, 0), (0, EMBEDDING_COMPONENTS - matrix.shape[1])),
                mode="constant",
            )
        return matrix[:, :EMBEDDING_COMPONENTS]

    def _single_document_vector(self, document: str, pipeline: dict[str, Any] | None) -> np.ndarray:
    ...
    def _history_vector(self, raw_value: str, pipeline: dict[str, Any] | None) -> np.ndarray:
        docs = self._parse_history_documents(raw_value)
        return self._history_vector_from_docs(docs, pipeline)
    ...
    def _macro_similarity(self, instance: dict[str, Any]) -> float:
        protein_g = _safe_float(instance.get("protein_g"))
        carbohydrate_g = _safe_float(instance.get("carbohydrate_g"))
        total_fat_g = _safe_float(instance.get("total_fat_g"))
        candidate = np.array(
            [
        ...
        history = np.array(
            [
                _safe_float(instance.get("user_history_avg_calories")),
                _safe_float(instance.get("user_history_avg_protein_g")),
                _safe_float(instance.get("user_history_avg_carbohydrate_g")),
                _safe_float(instance.get("user_history_avg_total_fat_g")),
            ],
            dtype=float,
        )
