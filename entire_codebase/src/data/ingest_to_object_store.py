...
def sha256_file(path):
    '''
    SHA-256 hash computed before upload, stored as object metadata for reproducibility
    The SHA-256 hash here becomes the foundation for the versioning strategy used later in the batch pipeline.
    '''
    h = hashlib.sha256()
...
def validate_recipes(df):
    '''
    checks required columns, nulls, uniqueness of recipe_id
    '''
    print("\n-- Validating recipes --")
...
def validate_interactions(df):
    '''
    checks rating range [1–5], required fields
    '''
...
def expand_recipes(df):
    '''
    if dataset < 5GB, it doubles recipes with Gaussian perturbation on numeric columns, as synthetic data augmentation.
    '''
